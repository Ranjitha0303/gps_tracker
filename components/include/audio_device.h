...
typedef struct{
    int pa_type;
    int gpio_num;
    nwy_audio_pa_mode mode;
    int pa_init;
    int headset_state; // 1: headset plugin; 0:headset plugout
    int enable_level;  // 1: low enable,high disable; 0: high enable, low disable
}nwy_pa_control;
...
/* Begin: Add by leoshen for mic&spk volume adjustment func in 2022.10.25 */
uint16_t nwy_get_CodecGain(bool mode);
void nwy_set_CodecGain(bool mode, uint16_t Volume);
#endif

/* End: Add by leoshen for mic&spk volume adjustment func in 2022.10.25 */
#ifdef FEATURE_NWY_AUDIO_ENABLE_ANAGAIN
void nwy_set_ana_gain(int16_t vol);
#endif
OSI_EXTERN_C_END
#endif
