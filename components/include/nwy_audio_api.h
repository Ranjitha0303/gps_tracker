...
#endif
#endif /* __cplusplus */
typedef int(* record_event_handler)(char *pdata, unsigned int len);
...
#ifdef FEATURE_NWY_AUDIO_REG_CB
typedef void (*audio_start_cb)(void);
void nwy_audio_start_cb_regist(audio_start_cb start_cb);
#endif

#ifdef FEATURE_NWY_AUDIO_PA_AUTO_SET
void nwy_audio_pa_auto_enable(int enable, uint32_t gpio_num, uint32_t delay_start, uint32_t early_stop);
#endif

#ifdef FEATURE_NWY_AUDIO_PA_AUTO_SET_DUAL_GPIO
void nwy_audio_pa_auto_enable_2(int enable, int gpio_num0, int gpio_num1, uint32_t delay_start, uint32_t early_stop, uint32_t pa_mode);
#endif
...
#ifdef CONFIG_DTMF_OPEN_SUPPORT
void nwy_audio_tone_detect(bool enable, keytonedetectHook_t func);
#endif
...
#ifndef FEATURE_NWY_AUDIO_RM_API
/**
...
#ifdef FEATURE_NWY_AUDIO_PLAY_STATUS
  void nwy_audio_play_status(int *status,char** filename);
/**
 * @brief play status audio file
 *
