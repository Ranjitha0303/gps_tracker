...
typedef struct nwy_ftp_login_para_t
{
...
int nwy_ftp_login(nwy_ftp_login_para_t *ftp_param, resultcb cb);
...
@param
    uint16_t channel,[in]: channel (1-7)
    const char* filename [in]: The file name to get
    uint8_t type [in]: File transfer mode 1��ASCII 2��Binary
...
int nwy_ftp_get(uint16_t channel,const char* filename, uint8_t type, int offset, int len);
...
int nwy_ftp_put(uint16_t channel,const char* filename, uint8_t type ,uint8_t mode, const char *data, int len);
...
int nwy_ftp_filesize(uint16_t channel,const char* filename, uint16_t tout);
...
int nwy_ftp_logout(uint16_t channel,uint16_t tout);
